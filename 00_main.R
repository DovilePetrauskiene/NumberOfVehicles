...
poppass <- read.csv("input/Cities pop.csv", check.names = F, stringsAsFactors = F, sep=";")
reg.pop <- read.csv("input/Region pop.csv", check.names = F, stringsAsFactors = F, sep=";")
...
for (cc in unique(totals$RegionCode)){
  ...
  if (length(st[, year][!is.na(st[, year])])==1){
    ...
    st[, year] <- MASplineVector(as.numeric(st[,year])/as.numeric(s.pop), plot=T)*as.numeric(s.pop)
  }
    ...
    if (cc %in% expf[expf$exp=="connect",]$CityCode){
      ...
      for (j in s$ProductID){
        ...
        if ((length(s[s$ProductID==j,year][!is.na(s[s$ProductID==j,year])]))==1){
          ...
          s.out <- MASplineVector(s[s$ProductID==j,year]/st[,year])
...
# saving data
write.csv(data.out.proc, "output/Number of Vechiles final output proc.csv", row.names=F)
write.csv(data.out.num, "output/Number of Vechiles final output.csv", row.names=F)
